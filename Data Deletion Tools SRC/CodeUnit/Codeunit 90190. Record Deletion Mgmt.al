codeunit 90190 "Record Deletion Mgmt"
{
    ...
    PROCEDURE ClearRecordsToDelete(TableAction: Enum "Table Actions");
    VAR
        RecordDeletionTable: Record "seperate Record Deletion Table";
    BEGIN
        RecordDeletionTable.Reset();
        RecordDeletionTable.setrange("Delete Records", true);
        RecordDeletionTable.SetRange("Table Action", TableAction);
        RecordDeletionTable.MODIFYALL("Delete Records", false);
    END;

    PROCEDURE SetrecordToDelete(TableAction: Enum "Table Actions");
    VAR
        RecordDeletionTable: Record "seperate Record Deletion Table";
    BEGIN
        RecordDeletionTable.Reset();
        RecordDeletionTable.setfilter("No. of Records", '<>%1', 0);
        RecordDeletionTable.SetRange("Table Action", TableAction);
        RecordDeletionTable.MODIFYALL("Delete Records", True);
    END;

    PROCEDURE ClearRecordsToDeleteWithParems(TableAction: Enum "Table Actions");
    VAR
        RecordDeletionTable: Record "seperate Record Deletion Table";
    BEGIN
        RecordDeletionTable.Reset();
    ...
    PROCEDURE DeleteRecordsWithParems(TableAction: Enum "Table Actions");
    VAR
        RecRef: RecordRef;
        RecordDeletionTable: Record "Seperate Record Deletion Table";
        RecordDelTableRelError: Record "Records Del. Table Rel. Error";
    ...
    procedure InsertFromJson(JsonInput: Text)
    var
        JObj: JsonObject;
        JArray: JsonArray;
        JToken: JsonToken;
    begin
        ...
        if JArray.ReadFrom(JsonInput) then begin
            if JArray.Count() = 0 then
                Error('JSON array is empty. No records to import.');

            foreach JToken in JArray do begin
                if not JToken.IsObject() then
                    Error('Invalid JSON array element. Each item must be a JSON object {}.');
                InsertSingleRecord(JToken.AsObject());
            end;
        end
        // Fallback: try as single object
        else if JObj.ReadFrom(JsonInput) then
            InsertSingleRecord(JObj)
        else
            Error('Invalid JSON format. Input must be a JSON object {} or an array of objects [{}].\Received: %1', CopyStr(JsonInput, 1, 100));
    end;


    procedure InsertSingleRecord(JObj: JsonObject)
    var
        SepRecDel: Record "Seperate Record Deletion Table";
        JToken: JsonToken;
        TableId: Integer;
    begin
        // Validate required fields
        if not JObj.Get('tableId', JToken) then
            Error('Missing required field: "tableId".');
        TableId := JToken.AsValue().AsInteger();
        if TableId <= 0 then
            Error('Invalid value for "tableId": %1. Must be a positive integer.', TableId);

        if not JObj.Get('company', JToken) then
            Error('Missing required field: "company".');
        if JToken.AsValue().AsText() = '' then
            Error('"company" field cannot be empty.');

        // if not JObj.Get('tableType', JToken) then
        //     Error('Missing required field: "tableType".');
        // if JToken.AsValue().AsText() = '' then
        //     Error('"tableType" field cannot be empty.');

        if not JObj.Get('tableAction', JToken) then
            Error('Missing required field: "tableAction".');
        if JToken.AsValue().AsText() = '' then
            Error('"tableAction" field cannot be empty.');

        // if not JObj.Get('tableOrder', JToken) then
        //     Error('Missing required field: "tableOrder".');
        // if JToken.AsValue().AsText() = '' then
        //     Error('"tableOrder" field cannot be empty.');

        // All validated — now insert
        SepRecDel.Init();
        SepRecDel."Table ID" := TableId;

        JObj.Get('company', JToken);
        SepRecDel."Company" := CopyStr(JToken.AsValue().AsText(), 1, MaxStrLen(SepRecDel."Company"));

        // JObj.Get('tableType', JToken);
        // SepRecDel."Table Type" := ParseTableType(JToken.AsValue().AsText());

        JObj.Get('tableAction', JToken);
        SepRecDel."Table Action" := ParseTableAction(JToken.AsValue().AsText());

        // JObj.Get('tableOrder', JToken);
        // SepRecDel."Table Order" := ParseTableOrder(JToken.AsValue().AsText());

        SepRecDel.Insert(true);
    end;

    // local procedure ParseTableType(Value: Text): Enum "Table Type"
    // begin
    //     case UpperCase(Value) of
    //         'BCS':
    //             exit(Enum::"Table Type"::BCS);
    //         'LSS':
    //             exit(Enum::"Table Type"::LSS);
    //         'BCM':
    //             exit(Enum::"Table Type"::BCM);
    //         'LSM':
    //             exit(Enum::"Table Type"::LSM);
    //         'BCT':
    //             exit(Enum::"Table Type"::BCT);
    //         'LST':
    //             exit(Enum::"Table Type"::LST);
    //         'BCC':
    //             exit(Enum::"Table Type"::BCC);
    //         'LSC':
    //             exit(Enum::"Table Type"::LSC);
    //         else
    //             Error('Unknown Table Type: %1', Value);
    //     end;
    // end;

    local procedure ParseTableAction(Value: Text): Enum "Table Actions"
    begin
        case Value of
            'BC-Setups':
                exit(Enum::"Table Actions"::"BC-Setups");
            'LS-Setups':
                exit(Enum::"Table Actions"::"LS-Setups");
            'BC-Masters':
                exit(Enum::"Table Actions"::"BC-Masters");
            'LS-Masters':
                exit(Enum::"Table Actions"::"LS-Masters");
            'BC-Transactions':
                exit(Enum::"Table Actions"::"BC-Transactions");
            'LS-Transactions':
                exit(Enum::"Table Actions"::"LS-Transactions");
            'BC-Custom':
                exit(Enum::"Table Actions"::"BC-Custom");
            'LS-Custom':
                exit(Enum::"Table Actions"::"LS-Custom");
            else
                Error('Unknown Table Action: %1', Value);
        end;
    end;

    // local procedure ParseTableOrder(Value: Text): Enum "Table Order"
    // begin
    //     case UpperCase(Value) of
    //         'ZERO', '0':
    //             exit(Enum::"Table Order"::Zero);
    //         'ONE', '1':
    //             exit(Enum::"Table Order"::One);
    //         'TWO', '2':
    //             exit(Enum::"Table Order"::Two);
    //         'THREE', '3':
    //             exit(Enum::"Table Order"::Three);
    //         'FOUR', '4':
    //             exit(Enum::"Table Order"::Four);
    //         'FIVE', '5':
    //             exit(Enum::"Table Order"::Five);
    //         'SIX', '6':
    //             exit(Enum::"Table Order"::Six);
    //         'SEVEN', '7':
    //             exit(Enum::"Table Order"::Seven);
    //         'EIGHT', '8':
    //             exit(Enum::"Table Order"::Eight);
    //         else
    //             Error('Unknown Table Order: %1', Value);
    //     end;
    // end;
}
